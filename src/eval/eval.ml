...
let num_bin_op left right op =
  match (left, right) with
  | NumberVal i1, NumberVal i2 -> NumberVal (op i1 i2)
  | _ -> raise EvalError

let bool_bin_op left right op =
  match (left, right) with
...
let rec eval expr env =
  ...
  | Letrec (bindings, body) ->
      let new_env = ExtendedRec (bindings, env) in
      eval body new_env
...
and eval_op op env =
  ...
  | Debug expr ->
      print_endline (show_exprval (eval expr env));
      UnitVal

and apply_closure cls value =
  match cls with
  | ClosureVal (args, Procedure (paras, body, env)) ->
      let values = List.cons (to_denval value) args in
      if List.length values = List.length paras then
        let new_env = extend_envs paras (List.rev values) env in
        eval body new_env
      else ClosureVal (values, Procedure (paras, body, env))
      (* Curry *)
  | _ -> raise EvalError
...
let eval_program pg = match pg with Program expr -> eval expr (init_mapenv ())
...
let parse_file file =
  let open Parsing in
  let lexbuf = open_in file |> Lexing.from_channel in
  Parser.program Lexer.read_token lexbuf
